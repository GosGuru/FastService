...
- **2026-08-12:** The first post-cutover smoke revealed that Vercel still held the old Supabase publishable key, so public routes fell back to local seed content. After persisting the replacement key and canonical `NEXT_PUBLIC_SITE_URL`, production was verified to render recovered R2-backed content and canonical sitemap URLs.
