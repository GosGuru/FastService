...
- **2026-08-12:** The first post-cutover smoke revealed that Vercel still held the old Supabase publishable key, so public routes fell back to local seed content. The replacement key and canonical `NEXT_PUBLIC_SITE_URL` were persisted and timestamp-verified in Vercel; verify recovered R2-backed content on the deployment created after those updates before closing the incident.
