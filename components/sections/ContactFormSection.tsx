...
const contactCopy: Record<Locale, ContactCopy> = {
	es: {
		...
		description:
			"Déjanos los datos básicos y te respondemos por WhatsApp con disponibilidad real, opciones y el siguiente paso claro.",
		...
		message: "Mensaje",
		privacy:
			"Acepto que FastServices me contacte para gestionar esta solicitud.",
	...
	en: {
		...
		description:
			"Leave the basics and we will reply by WhatsApp with real availability, options and the next clear step.",
		...
		messageGreeting: "Hello, I would like advice for an Ibiza experience.",
	},
	de: {
		...
		description:
			"Sende uns die wichtigsten Daten. Wir antworten per WhatsApp mit echter Verfugbarkeit, Optionen und dem nachsten Schritt.",
		...
		message: "Nachricht",
		privacy:
			"Ich stimme zu, dass FastServices mich zur Bearbeitung dieser Anfrage kontaktieren darf.",
		submit: "Anfrage senden",
		sent: "WhatsApp wurde mit deiner Anfrage geoffnet.",
		required: "Pflichtfeld",
		messageGreeting: "Hallo, ich möchte Beratung für ein Erlebnis auf Ibiza.",
	},
	nl: {
		...
		description:
			"Laat de basisgegevens achter. We antwoorden via WhatsApp met echte beschikbaarheid, opties en een duidelijke volgende stap.",
		...
		message: "Bericht",
		privacy:
			"Ik ga ermee akkoord dat FastServices contact met mij opneemt om deze aanvraag te beheren.",
		submit: "Aanvraag sturen",
		sent: "We hebben WhatsApp geopend met je aanvraag klaar om te verzenden.",
		required: "Verplicht veld",
		messageGreeting: "Hallo, ik wil graag advies voor een ervaring op Ibiza.",
	},
	ru: {
		...
		description:
			"Оставьте основные данные, и мы ответим в WhatsApp с реальной доступностью, вариантами и четким следующим шагом.",
		...
		message: "Сообщение",
		privacy:
			"Я согласен, что FastServices может связаться со мной для обработки этого запроса.",
...
function buildContactMessage(
	...
	const greeting = labels.messageGreeting;
...
export function ContactFormSection({ locale }: ContactFormSectionProps) {
	...
	function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		window.open(
			buildWhatsAppUrl(whatsappMessage, locale, phone),
			"_blank",
			"noopener,noreferrer",
		);
		setSent(true);
		...
		<section className="contact-section" id="contact-form">
			<div className="container contact-section__inner">
				<div className="contact-section__content">
					...
					<div
						className="contact-section__methods"
					...
					>
						...
						<dl>
							<div>
								<dt>
									<FaWhatsapp aria-hidden="true" /> {labels.whatsapp}
								</dt>
								<dd>
									<Link
							...
							<div>
								<dt>
									<FiMail aria-hidden="true" /> {labels.email}
								</dt>
								<dd>
									<Link href="mailto:Fastservicesibiza@gmail.com">
										Fastservicesibiza@gmail.com
									</Link>
								</dd>
							</div>
						</dl>
				...
				<form className="contact-request-form" onSubmit={handleSubmit}>
					<div className="contact-request-form__grid">
						...
						<div className="contact-phone-group">
							...
							<label className="contact-field contact-field--phone">
								<span>
									<FiPhone aria-hidden="true" /> {labels.phone}*
								</span>
								<input
									name="phone"
									type="tel"
						...
						<label className="contact-field">
							<span>
								<FiUsers aria-hidden="true" /> {labels.passengers}
							</span>
						...
						<label className="contact-field">
							<span>
								<FiCalendar aria-hidden="true" /> {labels.service}
							</span>
					...
					<label className="contact-consent">
						...
						/>
						<span>
							<FiShield aria-hidden="true" /> {labels.privacy}
						</span>
					</label>
