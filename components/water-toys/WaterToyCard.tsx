...
export function WaterToyCard({ toy, locale, sectionSlug }: WaterToyCardProps) {
	const href = sectionSlug
		? `/${locale}/${sectionSlug}/${getLocalizedSlug(toy.slugsByLocale, locale)}`
		: getWaterToyPath(toy.id, locale);

	return (
		<article className="water-toy-card">
			<ImageCarousel
				assets={[toy.image, ...toy.gallery]}
				locale={locale}
				href={href}
				ariaLabel={getLocalizedValue(toy.name, locale)}
				className="water-toy-card__image"
			...
			<div className="water-toy-card__body">
				...
				<CardActions
					locale={locale}
					whatsappMessage={getLocalizedValue(toy.whatsappMessage, locale)}
					detailHref={href}
					showDetail={false}
				/>
			</div>
