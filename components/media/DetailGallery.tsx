...
import { applyTemplate, uiLabels, type Locale } from "@/lib/i18n";
...
export function DetailGallery({ assets, locale, title }: DetailGalleryProps) {
	...
	function scrollToImage(index: number) {
		...
		const target = railRef.current?.children.item(
			nextIndex,
		) as HTMLElement | null;
	...
	function handleScroll() {
		...
		Array.from(rail.children).forEach((child, index) => {
			const distance = Math.abs(
				(child as HTMLElement).getBoundingClientRect().left - railLeft,
			);
	...
	return (
		<>
			<section
				className="boat-detail-gallery"
				aria-labelledby="boat-detail-gallery-title"
			>
				<div className="container boat-detail-gallery__header">
					<h2 id="boat-detail-gallery-title" className="detail-section-title">
						{title}
					</h2>
				</div>
				...
				<div className="container boat-detail-gallery__container">
					<div
						className="boat-detail-gallery__viewport"
						ref={railRef}
					...
					>
						{images.map((asset, index) => (
							<figure className="boat-detail-gallery__item" key={asset.src}>
								<button
									type="button"
									className="boat-detail-gallery__trigger"
									onClick={() => {
										setActiveIndex(index);
										setIsFullscreenOpen(true);
									}}
									aria-label={applyTemplate(labels.galleryOpenFullscreen, {
								...
								>
									...
									/>
									<span
										className="boat-detail-gallery__expand"
										aria-hidden="true"
									>
										<FiMaximize2 />
									</span>
					...
					<div
						className="boat-detail-gallery__controls"
						aria-label={labels.galleryControlsAriaLabel}
					>
						...
						<div className="boat-detail-gallery__nav">
							<button
								...
								aria-label={labels.galleryPreviousImage}
							>
							...
							<button
								...
								aria-label={labels.galleryNextImage}
							>
			...
			{isFullscreenOpen && activeImage ? (
				...
				>
					<button
						...
						aria-label={labels.galleryCloseImage}
					/>
					...
					{total > 1 ? (
						<>
							...
							<div
								className="media-lightbox__thumbs"
								aria-label={labels.galleryAvailableImages}
							>
								{images.map((item, index) => (
									<button
										...
										aria-label={applyTemplate(labels.galleryViewImage, {
											index: index + 1,
										})}
									/>
								))}
