...
import {
	buildAlternates,
	getAllLocalizedItemPaths,
	getItemBySectionAndSlug,
	...
	return {
		...
		alternates: {
			...
			languages: buildAlternates(
				section.slugsByLocale,
				`/${getLocalizedSlug(item.slugsByLocale, locale)}`,
			),
		},
	};
}
...
export default async function ItemPage({ params }: Props) {
	...
	const canonicalSectionSlug = section
		? getLocalizedSlug(section.slugsByLocale, locale)
		: slug;
	...
	const title =
		item.kind === "vehicle" ? item.name : getLocalizedValue(item.name, locale);
	const summary =
		item.kind === "vehicle"
			? getLocalizedValue(item.overview, locale)
			: getLocalizedValue(item.details, locale);
	const richDescription = item.richDescription?.[locale] ??
		item.richDescription?.es ?? {
			html: summary ? `<p>${escapeHtml(summary)}</p>` : "",
			text: summary,
		};
	const specs = item.specs ?? [];
	const priceText = item.priceLabel
		? getLocalizedValue(item.priceLabel, locale).trim()
		: "";
	const marina = item.marina ? getLocalizedValue(item.marina, locale) : "";
	const amenities = (
		item.kind === "vehicle"
			? (item.amenities ?? item.services)
			: (item.amenities ?? [])
	).flatMap((amenity) => {
	...
	const eyebrow =
		item.kind === "vehicle" ? t("transfer", locale) : t("waterToy", locale);
			...
			<section className="boat-detail-hero">
				...
				<div className="container boat-detail-hero__inner">
					<div className="boat-detail-hero__content">
						<Link href={sectionHref} className="boat-detail-hero__tag">
							{eyebrow}
						</Link>
						<h1>{title}</h1>
					...
					<div className="container boat-detail-info__grid">
						...
						>
							...
							<div className="boat-detail-reserve">
								<WhatsAppCta
									locale={locale}
									message={getLocalizedValue(item.whatsappMessage, locale)}
