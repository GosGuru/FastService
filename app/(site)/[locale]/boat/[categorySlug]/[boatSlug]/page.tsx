...
type Props = {
	params: Promise<{ locale: string; categorySlug: string; boatSlug: string }>;
};
...
const collectionLabels: Record<string, Record<string, string>> = {
	...
	"fast-boats": {
		es: "Embarcaciones rápidas",
		en: "Fast Boats",
		de: "Schnellboote",
		nl: "Snelle boten",
		ru: "Быстроходные катера",
	},
};
...
const uiCopy: Record<string, Record<string, string>> = {
	...
	video: { es: "Video", en: "Video", de: "Video", nl: "Video", ru: "Видео" },
	relatedPrefix: {
		es: "Otros",
		en: "Other",
		de: "Weitere",
...
export default async function BoatPage({ params }: Props) {
	...
	const canonicalCategorySlug = getLocalizedSlug(
		boat.categorySlugsByLocale,
		locale,
	);
	...
	if (
		...
		boatSlug !== canonicalBoatSlug
	) {
		redirect(`/${locale}/boat/${canonicalCategorySlug}/${canonicalBoatSlug}`);
	}

	const content = await getPublicContent();
	const related = (await getBoatsByCollection(boat.collectionId))
	...
	const collectionLabel =
		collectionLabels[boat.collectionId]?.[locale] ??
		collectionLabels[boat.collectionId]?.es ??
		"";
		...
		<main>
			<section className="boat-detail-hero">
				<MediaImage
					...
					sizes="100vw"
					priority
					className="boat-detail-hero__media"
				/>
				<div className="container boat-detail-hero__inner">
					<div className="boat-detail-hero__content">
						<Link href={collectionHref} className="boat-detail-hero__tag">
							{collectionLabel}
						</Link>
						<h1>{boat.name}</h1>

						{boat.priceLabel && (
							<p className="boat-detail-hero__price">
								{getLocalizedValue(boat.priceLabel, locale)}
							</p>
						)}
						...
						<div className="boat-detail-hero__specs">
							{boat.specs.map((spec) => {
								const Icon =
									detailSpecIcons[spec.icon as keyof typeof detailSpecIcons] ??
									PiRuler;
				...
				<section className="boat-detail-info">
					<div className="container boat-detail-info__grid">
						...
						<aside
							...
							aria-label={`${boat.name} ${t("equipment")}`}
						>
							{amenities.length > 0 ? (
			...
			{boat.video?.src ? (
				<section
					className="boat-detail-video-section"
					aria-labelledby="boat-detail-video-title"
				>
					<div className="container">
						<h2 id="boat-detail-video-title" className="detail-section-title">
							{videoTitle}
						</h2>
						<figure className="boat-detail-video">
							<video
								controls
								preload="metadata"
								poster={boat.image.src}
								aria-label={videoTitle}
							>
								<source
									src={boat.video.src}
									type={boat.video.mimeType ?? "video/mp4"}
								/>
							</video>
						</figure>
					</div>
			...
			{boatFaqs.length > 0 && <FaqSection items={boatFaqs} locale={locale} />}
